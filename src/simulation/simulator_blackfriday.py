...
class SimulatorBlackFridayBaseline:
    ...
    def __init__(self, config, metrics_class, arrival_rng, choice_rng, service_rng, lambda_function):
        self.config = config
        ...
        self.worker_nodes = [WorkerNode(env=self.env, node_id=i, queue_instance=simpy.Store(self.env)) for i in range(config.NUM_WORKERS)]
    ...
    def request_generator(self):
        ...
        while True:
            ...
            req_id_counter += 1
            new_request = Request(request_id=req_id_counter, req_type=chosen_type, arrival_time=self.env.now, timeout=type_timeout, service_time=service_time)
    ...
    def metrics_recorder(self):
        ...
        while True:
            # Registrazione delle metriche (invariata)
    ...
    def run(self, simulation_duration: float):
        ...
        for worker in self.worker_nodes: self.scale_worker(worker, self.config.INITIAL_PODS_PER_WORKER)
        if self.config.HPA_ENABLED: HPA(self.env, self)
        self.env.run(until=simulation_duration)

# --- Simulatore per lo scenario DWFQ ---
# Questa classe ora eredita correttamente da SimulatorWFQ, riutilizzandone tutta la logica.
class SimulatorBlackFriday(SimulatorWFQ):
    def __init__(self, config, metrics_class, arrival_rng, choice_rng, service_rng, lambda_function):
        # Chiama il costruttore della classe padre (SimulatorWFQ) per inizializzare tutto.
        super().__init__(config, metrics_class, arrival_rng, choice_rng, service_rng, lambda_function)