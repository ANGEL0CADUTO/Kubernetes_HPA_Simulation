...
class PlotterWFQ:
    ...
    def generate_final_dashboards(self, output_dir, run_prefix, peak_start, peak_end, base_load, peak_load):
        ...
        # --- SEZIONE TIMEOUT (CON SCALA CORRETTA) ---
        # Impostiamo un limite più alto per vedere i picchi
        TIMEOUT_Y_AXIS_LIMIT = max(base_load, peak_load)
        ...
        ax7.set_xlabel("Tempo (s)"); ax7.set_ylabel("Tasso di Timeout (req/s)"); ax7.grid(True); ax7.set_ylim(bottom=0, top=TIMEOUT_Y_AXIS_LIMIT)
        base_ts_high = [ts for ts, rt in self.metrics_base.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.HIGH]
        ...
        ax_load7 = ax7.twinx(); ax_load7.plot(load_times, load_values, color='gray', linestyle=':', lw=2, alpha=0.7, label='Carico'); ax_load7.set_ylabel("Carico (req/s)", color='gray')
        ...
        ax8.set_xlabel("Tempo (s)"); ax8.set_ylabel("Tasso di Timeout (req/s)"); ax8.grid(True); ax8.set_ylim(bottom=0, top=TIMEOUT_Y_AXIS_LIMIT)
        base_ts_medium = [ts for ts, rt in self.metrics_base.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.MEDIUM]
        prio_ts_medium = [ts for ts, rt in self.metrics_prio.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.MEDIUM]
        wfq_ts_medium = [ts for ts, rt in self.metrics_wfq.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.MEDIUM]
        ...
        ax9.set_xlabel("Tempo (s)"); ax9.set_ylabel("Tasso di Timeout (req/s)"); ax9.grid(True); ax9.set_ylim(bottom=0, top=TIMEOUT_Y_AXIS_LIMIT)
        base_ts_low = [ts for ts, rt in self.metrics_base.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.LOW]
        prio_ts_low = [ts for ts, rt in self.metrics_prio.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.LOW]
        wfq_ts_low = [ts for ts, rt in self.metrics_wfq.timeout_history if self.config.REQUEST_TYPE_TO_PRIORITY.get(rt) == self.config.Priority.LOW]
