# File: analysis/plotter_wfq.py (VERSIONE FINALE COMPLETA)
...
class PlotterWFQ:
    ...
    def generate_final_dashboards(self, output_dir, run_prefix, peak_start, peak_end, base_load, peak_load):
        ...
        # --- Grafico 3: Performance Interna del WFQ (Aggiuntivo) ---
        fig3, ax3 = plt.subplots(figsize=(20, 8)); ax3.set_title(f"Performance Interna del WFQ - {run_prefix}", fontsize=18)
        ...
        for prio in config.Priority:
            plot_series(ax3, wfq_ma_data.get(prio), color=colors.get(prio, 'black'), lw=2, label=f'WFQ - {prio.name}')
