...
class PlotterWFQ:
    ...
    def _plot_aggregated_response_time_trends(self, output_dir, run_prefix, peak_start, peak_end, base_load, peak_load):
        ...
        for req_type, history in self.metrics_base_agg.response_times_history.items():
            if self.config.REQUEST_TYPE_TO_PRIORITY.get(req_type) == config.Priority.HIGH:
                base_history_high_only.extend(history)
        base_history_high_only.sort(key=lambda x: x[0])
        base_ma_high = get_ma(base_history_high_only)

        # Per il grafico di starvation, la media aggregata del baseline è ancora un confronto utile.
        base_ma_all = get_ma(self.metrics_base_agg.get_all_response_times_with_timestamps())

        prio_ma_high = get_ma(self.metrics_prio_agg.response_times_history_by_prio.get(config.Priority.HIGH, []))
        prio_ma_low = get_ma(self.metrics_prio_agg.response_times_history_by_prio.get(config.Priority.LOW, []))
        wfq_ma_high = get_ma(self.metrics_wfq_agg.response_times_history_by_prio.get(config.Priority.HIGH, []))
        wfq_ma_low = get_ma(self.metrics_wfq_agg.response_times_history_by_prio.get(config.Priority.LOW, []))
        ...
        ax2.set_title(f"Analisi Starvation Aggregata (Cluster) per Priorità LOW - {run_prefix}", fontsize=18)
        ax2.set_xlabel("Tempo (s)"); ax2.set_ylabel("Tempo Risposta Medio (s)"); ax2.grid(True); ax2.set_ylim(bottom=0, top=15.0)

        plot_series(ax2, base_ma_all, color='royalblue', linestyle='--', lw=2.5, label='Baseline (FIFO) - Media Totale') # Qui usiamo ancora la media totale come riferimento
        plot_series(ax2, prio_ma_low, color='darkred', lw=2, label='Priorità Strette - Solo LOW')
        plot_series(ax2, wfq_ma_low, color='limegreen', lw=2, label='WFQ - Solo LOW')
    ...
    def _plot_aggregated_throughput_per_priority(self, output_dir, run_prefix):
        ...
        for req_type, req_list in self.metrics_base_agg.response_times_data.items():
            ...
            base_served_by_prio[prio] += len(req_list)
        ...
        sns.barplot(data=df, x='Priorità', y='Conteggio', hue='Scenario',
                    order=[p.name for p in config.Priority],
                    hue_order=['Baseline (FIFO)', 'Priorità Strette', 'WFQ'],
                    palette={'Baseline (FIFO)': 'royalblue', 'Priorità Strette': 'darkred', 'WFQ': 'limegreen'}, ax=ax)

        ax.set_title(f"Throughput Aggregato (Cluster) per Priorità - {run_prefix}", fontsize=18)
        ax.set_xlabel("Classe di Priorità", fontsize=14)
        ax.set_ylabel("Numero Totale di Richieste Servite", fontsize=14)

        for container in ax.containers:
            ax.bar_label(container, fmt='%d', padding=3, fontsize=9)

        ax.legend(title='Scenario'); ax.grid(True, axis='y', linestyle='--', alpha=0.6)
        fig.tight_layout()
    ...
    def _plot_aggregated_timeout_rates_per_priority(self, output_dir, run_prefix, peak_start, peak_end, base_load, peak_load):
        fig, axes = plt.subplots(3, 1, figsize=(20, 24), sharex=True, sharey=True)
        sim_time = self.config.SIMULATION_TIME
        TIME_WINDOW_SEC = 10
        ...
        for i, prio in enumerate(priorities):
            ...
            ax.set_title(f"Tasso di Timeout Aggregato (Cluster) per Priorità {prio.name}", fontsize=16)
            ax.set_ylabel("Timeout / sec"); ax.grid(True); ax.legend(); ax.set_ylim(bottom=0)

            ax_load = ax.twinx()
            load_times = [0, peak_start, peak_start, peak_end, peak_end, sim_time]; load_values = [base_load, base_load, peak_load, peak_load, base_load, base_load]
            ax_load.plot(load_times, load_values, color='gray', linestyle=':', lw=2, alpha=0.7, label='Carico Applicato')
            ax_load.set_ylabel("Carico (req/s)", color='gray'); ax_load.set_ylim(bottom=0)

        axes[-1].set_xlabel("Tempo (s)")
        fig.suptitle(f"Analisi Tasso di Perdita Aggregato (Cluster) - {run_prefix}", fontsize=20, fontweight='bold')
    ...
    def _plot_hotspot_analysis_dashboards(self, output_dir, run_prefix, peak_start, peak_end, base_load, peak_load):
        ...
        metrics_w0_prio = self.metrics_per_worker_prio[worker_id_to_analyze]
        metrics_w0_wfq = self.metrics_per_worker_wfq[worker_id_to_analyze]

        fig1, ax1 = plt.subplots(figsize=(20, 8))
        ax1.set_title(f"Evoluzione Coda Locale su Hotspot (Worker {worker_id_to_analyze}) - {run_prefix}", fontsize=18)

        if metrics_w0_base.queue_length_history:
            times, lengths = zip(*metrics_w0_base.queue_length_history)
            ax1.plot(times, lengths, label='Baseline (FIFO)', color='royalblue', lw=2)
        if metrics_w0_prio.queue_lengths:
            ax1.plot(metrics_w0_prio.timestamps, metrics_w0_prio.queue_lengths, label='Priorità Strette', color='darkred', lw=2)
        if metrics_w0_wfq.queue_lengths:
            ax1.plot(metrics_w0_wfq.timestamps, metrics_w0_wfq.queue_lengths, label='WFQ', color='limegreen', lw=2)

        ax1.set_xlabel("Tempo (s)"); ax1.set_ylabel("N. Richieste in Coda Locale"); ax1.grid(True); ax1.legend()
        ax1.set_yscale('log'); ax1.set_ylim(bottom=1)
        self._save_plot(output_dir, f"{run_prefix}_5_hotspot_queue_length.png", fig1)

        fig2, ax2 = plt.subplots(figsize=(20, 8))
        ax2.set_title(f"Protezione QoS su Hotspot (Worker {worker_id_to_analyze}) - {run_prefix}", fontsize=18)
        ...
        for req_type, history in metrics_w0_base.response_times_history.items():
            if self.config.REQUEST_TYPE_TO_PRIORITY.get(req_type) == config.Priority.HIGH:
                base_w0_history_high_only.extend(history)
        base_w0_history_high_only.sort(key=lambda x: x[0])
        base_ma_w0_high = get_ma(base_w0_history_high_only)

        prio_ma_high_w0 = get_ma(metrics_w0_prio.response_times_history_by_prio.get(config.Priority.HIGH, []))
        wfq_ma_high_w0 = get_ma(metrics_w0_wfq.response_times_history_by_prio.get(config.Priority.HIGH, []))

        plot_series(ax2, base_ma_w0_high, color='royalblue', linestyle='--', lw=2.5, label='Baseline (FIFO) - Solo HIGH')
        plot_series(ax2, prio_ma_high_w0, color='darkred', lw=2, label='Priorità Strette - Solo HIGH')
        plot_series(ax2, wfq_ma_high_w0, color='limegreen', lw=2, label='WFQ - Solo HIGH')

        ax2.set_xlabel("Tempo (s)"); ax2.set_ylabel("Tempo Risposta Medio Locale (s)"); ax2.grid(True); ax2.legend()
        ax2.set_ylim(bottom=0, top=15.0)
        ...
        lines, labels = ax2.get_legend_handles_labels(); lines2, labels2 = ax_load.get_legend_handles_labels()
